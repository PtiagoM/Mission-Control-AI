...
- analisar temperatura, energia e comunicação;
- calcular automaticamente o status operacional;
...
def analisar_leitura(
    ...
    """
    Analisa uma leitura da missão e retorna:
...
def registrar_leitura(
    ...
    """
    analise = analisar_leitura(temperatura, energia, comunicacao)
...
def inserir_leitura_manual() -> None:
    ...
    print(f"\nLeitura registrada com sucesso. Status operacional: {leitura['status_operacional']}")
...
def simular_leitura_operacional() -> None:
    ...
    for indice, leitura in enumerate(leituras_simuladas, start=1):
        ...
        print(
            ...
            f"Status calculado: {analise['status_operacional']}"
        )
    ...
    while True:
        try:
            ...
            if 1 <= escolha <= len(leituras_simuladas):
                ...
                leitura = registrar_leitura(
                    temperatura=leitura_base["temperatura"],
                    energia=leitura_base["energia"],
                    comunicacao=leitura_base["comunicacao"],
                    origem=f"Simulação {escolha}",
                )

                print(f"\nLeitura simulada registrada. Status operacional: {leitura['status_operacional']}")
                return
...
def simular_evento_critico() -> None:
    ...
    print(f"Status operacional calculado: {leitura['status_operacional']}")
...
def visualizar_ultima_leitura() -> None:
    ...
    print("\n" + "-" * 70)
    print(f"LEITURA {leitura['numero']} — STATUS OPERACIONAL: {leitura['status_operacional']}")
    print("-" * 70)
...
def mostrar_historico() -> None:
    ...
    for leitura in historico_leituras:
        print(
            ...
            f"{leitura['status_operacional']:<20}"
        )
...
def exibir_resumo_geral() -> None:
    ...
    normais = sum(1 for leitura in historico_leituras if leitura["status_operacional"] == "NORMAL")
    atencoes = sum(1 for leitura in historico_leituras if leitura["status_operacional"] == "ATENÇÃO")
    criticos = sum(1 for leitura in historico_leituras if leitura["status_operacional"] == "CRÍTICO")
